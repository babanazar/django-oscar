...
AddServiceSerializer = get_api_class("serializers.service", "AddServiceSerializer")
(
...
class AddServiceView(APIView):
    """
    Add a certain quantity of a service to the basket.

    POST(url, quantity)
    {
        "url": "http://testserver.org/oscarapi/services/209/",
        "quantity": 6
    }

    If you've got some options to configure for the service to add to the
    ...
    add_service_serializer_class = AddServiceSerializer
    serializer_class = BasketSerializer

    def validate(
        self, basket, service, quantity, options
    ):  # pylint: disable=unused-argument
        availability = basket.strategy.fetch_for_service(service).availability

        # check if service is available at all
        if not availability.is_available_to_buy:
            return False, availability.message

        current_qty = basket.service_quantity(service)
    ...
    def post(self, request, *args, **kwargs):  # pylint: disable=redefined-builtin
        p_ser = self.add_service_serializer_class(
            data=request.data, context={"request": request}
        )
        if p_ser.is_valid():
            basket = operations.get_basket(request)
            service = p_ser.validated_data["url"]
            quantity = p_ser.validated_data["quantity"]
            options = p_ser.validated_data.get("options", [])

            basket_valid, message = self.validate(basket, service, quantity, options)
            if not basket_valid:
                return Response(
                    {"reason": message}, status=status.HTTP_406_NOT_ACCEPTABLE
                )

            basket.add_service(service, quantity=quantity, options=options)
...
class LineList(BasketPermissionMixin, generics.ListCreateAPIView):
    ...
    POST(basket, line_reference, service, stockrecord,
    ...
    Add a line to the basket, example::

        {
            ...
            "service": "http://127.0.0.1:8000/oscarapi/services/209/",
            "stockrecord":
                "http://127.0.0.1:8000/ooscarapi/services/209/stockercords/1/",
            "quantity": 3,
            "price_currency": "EUR",
