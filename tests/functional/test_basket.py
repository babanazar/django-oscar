...
class TestBasketMerging(TestCase):
    ...
    def setUp(self):
        self.service = create_service(num_in_stock=10)
        self.user_basket = Basket()
        self.user_basket.strategy = strategy.Default()
        add_service(self.user_basket, service=self.service)
        self.cookie_basket = Basket()
        self.cookie_basket.strategy = strategy.Default()
        add_service(self.cookie_basket, quantity=2, service=self.service)
    ...
    def test_merge_line_takes_max_quantity(self):
        line = self.user_basket.lines.get(service=self.service)
...
class AnonAddToBasketViewTests(WebTestCase):
    ...
    def setUp(self):
        ...
        url = reverse('basket:add', kwargs={'pk': self.service.pk})
        post_params = {'service_id': self.service.id,
                       'action': 'add',
                       'quantity': 1}
    ...
    def test_price_is_recorded(self):
        ...
        line = basket.lines.get(service=self.service)
        stockrecord = self.service.stockrecords.all()[0]
...
class BasketThresholdTest(WebTestCase):
    ...
    def test_adding_more_than_threshold_raises(self):
        dummy_service = create_service(price=D('10.00'), num_in_stock=10)
        url = reverse('basket:add', kwargs={'pk': dummy_service.pk})
        post_params = {'service_id': dummy_service.id,
                       'action': 'add',
                       'quantity': 2}
...
class SavedBasketTests(WebTestCase):
    ...
    def test_moving_to_saved_basket_updates_existing(self):
        ...
        saved_basket = factories.BasketFactory(owner=self.user,
                                               status=Basket.SAVED)
        saved_basket.add_service(service)
    ...
    def test_moving_from_saved_basket_more_than_stocklevel_raises(self):
        self.user = User.objects.create_user(username='test', password='pass',
                                             email='test@example.com')
        service = create_service(price=D('10.00'), num_in_stock=1)
        ...
        self.assertEqual(Basket.open.get(id=basket.id).lines.get(service=service).quantity, 1)
        self.assertRedirects(response, reverse('basket:summary'))
...
class BasketFormSetTests(WebTestCase):
    ...
    def test_formset_with_removed_line(self):
        services = [create_service() for i in range(3)]
        basket = factories.create_basket(empty=True)
        basket.owner = self.user
        basket.save()

        add_service(basket, service=services[0])
        add_service(basket, service=services[1])
        add_service(basket, service=services[2])
        response = self.get(reverse('basket:summary'))
        formset = response.context['formset']
        self.assertEqual(len(formset.forms), 3)

        basket.lines.filter(service=services[0]).delete()
    ...
    def test_invalid_formset_with_removed_line(self):
        ...
        stockrecord = services[1].stockrecords.first()
    ...
    def test_deleting_valid_line_with_other_valid_line(self):
        ...
        basket.owner = self.user
        basket.save()
        add_service(basket, service=service_1)
        add_service(basket, service=service_2)
    ...
    def test_deleting_valid_line_with_other_invalid_line(self):
        ...
        # Render service for other line out of stock
        service_2.stockrecords.update(num_in_stock=0)
    ...
    def test_deleting_invalid_line_with_other_valid_line(self):
        ...
        # Render service for to-be-deleted line out of stock
        service_1.stockrecords.update(num_in_stock=0)
    ...
    def test_deleting_invalid_line_with_other_invalid_line(self):
        ...
        # Render services for both lines out of stock
        service_1.stockrecords.update(num_in_stock=0)
        service_2.stockrecords.update(num_in_stock=0)
