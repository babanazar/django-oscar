...
class ImportSmokeTest(TestCase):
    ...
    def setUp(self):
        ...
        self.service = Service.objects.get(upc='9780115531446')

    def test_all_rows_are_imported(self):
        self.assertEqual(10, Service.objects.all().count())

    def test_class_is_created(self):
        try:
            ServiceClass.objects.get(name="Book")
        except Service.DoesNotExist:
            self.fail()

    def test_only_one_class_is_created(self):
        self.assertEqual(1, ServiceClass.objects.all().count())

    def test_item_is_created(self):
        try:
            Service.objects.get(upc="9780115531446")
        except Service.DoesNotExist:
            self.fail()

    def test_title_is_imported(self):
        self.assertEqual("Prepare for Your Practical Driving Test", self.service.title)
...
class ImportWithFlushTest(TestCase):
    ...
    def test_items_are_flushed_by_importer(self):
        upc = "0000000000000"
        create_service(price=D('10.00'), upc=upc)

        self.importer.handle(TEST_BOOKS_CSV)

        with self.assertRaises(Service.DoesNotExist):
            Service.objects.get(upc=upc)
