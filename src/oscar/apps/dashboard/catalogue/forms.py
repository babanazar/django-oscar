...
ServiceSelect = get_class('dashboard.catalogue.widgets', 'ServiceSelect')
...
class ServiceClassSelectForm(forms.Form):
    """
    Form which is used before creating a service to select it's service class
    """

    service_class = forms.ModelChoiceField(
        label=_("Create a new service of type"),
        empty_label=_("-- Choose type --"),
        queryset=ServiceClass.objects.all())

    def __init__(self, *args, **kwargs):
        """
        If there's only one service class, pre-select it
        """
        super().__init__(*args, **kwargs)
        qs = self.fields['service_class'].queryset
        if not kwargs.get('initial') and len(qs) == 1:
            self.fields['service_class'].initial = qs[0]


class ServiceSearchForm(forms.Form):
    upc = forms.CharField(max_length=64, required=False, label=_('UPC'))
    title = forms.CharField(
        max_length=255, required=False, label=_('Service title'))
...
class StockRecordForm(forms.ModelForm):
    ...
    def __init__(self, service_class, user, *args, **kwargs):
        ...
        if not service_class.track_stock:
...
def _attr_entity_field(attribute):
    # Service entities don't have out-of-the-box supported in the ServiceForm.
...
class ServiceForm(SEOFormMixin, forms.ModelForm):
    FIELD_FACTORIES = {
    ...
    def __init__(self, service_class, data=None, parent=None, *args, **kwargs):
        self.set_initial(service_class, parent, kwargs)
        super().__init__(data, *args, **kwargs)
        if parent:
            self.instance.parent = parent
            # We need to set the correct service structures explicitly to pass
            # attribute validation and child service validation. Note that
            # those changes are not persisted.
            self.instance.structure = Service.CHILD
            self.instance.parent.structure = Service.PARENT

            self.delete_non_child_fields()
        else:
            # Only set service class for non-child services
            self.instance.service_class = service_class
        self.add_attribute_fields(service_class, self.instance.is_parent)

        if 'slug' in self.fields:
            self.fields['slug'].required = False
            self.fields['slug'].help_text = _('Leave blank to generate from service title')
        if 'title' in self.fields:
            self.fields['title'].widget = forms.TextInput(
                attrs={'autocomplete': 'off'})

    def set_initial(self, service_class, parent, kwargs):
        """
        Set initial data for the form. Sets the correct service structure
        and fetches initial values for the dynamically constructed attribute
        fields.
        """
        if 'initial' not in kwargs:
            kwargs['initial'] = {}
        self.set_initial_attribute_values(service_class, kwargs)
        if parent:
            kwargs['initial']['structure'] = Service.CHILD

    def set_initial_attribute_values(self, service_class, kwargs):
        """
        Update the kwargs['initial'] value to have the initial values based on
        the service instance's attributes
        """
        instance = kwargs.get('instance')
        if instance is None:
            return
        for attribute in service_class.attributes.all():
            try:
    ...
    def add_attribute_fields(self, service_class, is_parent=False):
        """
        For each attribute specified by the service class, this method
        dynamically adds form fields to the service form.
        """
        for attribute in service_class.attributes.all():
            field = self.get_attribute_field(attribute)
            if field:
                self.fields['attr_%s' % attribute.code] = field
                # Attributes are not required for a parent service
                if is_parent:
    ...
    def delete_non_child_fields(self):
        """
        Deletes any fields not needed for child services. Override this if
    ...
    def _post_clean(self):
        """
        Set attributes before ModelForm calls the service's clean method
...
class ServiceImageForm(forms.ModelForm):
    ...
    class Meta:
        model = ServiceImage
        fields = ['service', 'original', 'caption', 'display_order']
...
class ServiceRecommendationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'recommendation': ServiceSelect,
        }


class ServiceClassForm(forms.ModelForm):
...
class ServiceAttributesForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if attr_type in [ServiceAttribute.OPTION, ServiceAttribute.MULTI_OPTION] and not option_group:
